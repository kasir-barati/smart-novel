describe('TTS Review Page', () => {
  ...
  describe('with stubbed GraphQL (no login required)', () => {
    ...
    it('should display the review page with header and labels', () => {
      ...
      cy.contains(
        'Review and edit the generated TTS-friendly',
      ).should('be.visible');
  ...
  describe('authenticated flow (writer login)', () => {
    beforeEach(() => {
      cy.login();
    });
    ...
    it('should show Generate TTS buttons on the novel chapter list', () => {
      ...
      cy.get('a[href^="/novel/"]', { timeout: 10000 })
        .first()
        .click();
