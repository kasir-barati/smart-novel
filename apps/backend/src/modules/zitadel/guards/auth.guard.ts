...
import { ZitadelSession as AuthSessionClass } from '../zitadel.session';

const NO_STRATEGY_ERROR =
  'Auth.js module must be imported to use AuthGuard';
...
function createAuthGuard(
  ...
  class MixinAuthGuard implements CanActivate {
    ...
    constructor(
      ...
      protected readonly options?: ZitadelModuleOptions,
      @Optional()
    ...
    getResponse(context: ExecutionContext): Response {
      return this.getOrCreateAdapter().getResponse(
        context,
      ) as Response;
    }
    ...
    /**
     * Lazily creates the Express HTTP adapter.
     * @returns The singleton instance of the `HttpAdapter`.
     */
    private getOrCreateAdapter(): HttpAdapter {
      if (!this.httpAdapter) {
    ...
    private async handlePublicRoute(
      ...
      const property =
        mergedOptions.property ?? defaultOptions.property;
    ...
    private async handleProtectedRoute(
      ...
      const publicSession =
        AuthSessionClass.fromCore(coreSession)?.toJSON();
      ...
      if (!publicSession?.user || error) {
        ...
        if (acceptsHtml) {
          ...
          const signInUrl =
            mergedOptions.pages?.signIn || '/auth/signin';
    ...
    private async getSession(
      ...
      const url = createActionURL(
        ...
        new Headers({
          host,
          'x-forwarded-host':
            (headers['x-forwarded-host'] as string) ?? host,
          'x-forwarded-proto':
