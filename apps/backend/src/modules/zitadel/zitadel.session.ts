...
import type { AuthUser, Session } from './interfaces';
 ...
 */
export class ZitadelSession implements Session {
   ...
   */
  static fromCore(
    core: CoreSession | null | undefined,
  ): ZitadelSession | null {
    return core ? new ZitadelSession(core) : null;
  }
  ...
  constructor(src: CoreSession | Partial<Session>) {
    ...
    if (srcAsRec) {
      copyExtraProps(
        srcAsRec,
        this,
        new Set<PropertyKey>(['user', 'expires']),
      );
    }
  }
   ...
   */
  with(patch: Partial<Session>): ZitadelSession {
